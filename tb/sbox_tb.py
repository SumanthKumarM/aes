...
# sbox_enb_n = {enb_n, _enb_n}. enb_n=0, _enb_n=1 enables the whole S-box
# to operate on the 128-bit state matrix (as opposed to the single-word
# mode used by AddRoundKey's KeyExpansion path).
SBOX_ENB_N_STATE_MATRIX = 0b01
...
async def signal_monitor(dut, label=""):
    ...
    while True:
        ...
        if cur["sbox_done_pulse"] == 1 and prev["sbox_done_pulse"] == 0:
            out_bytes = int_to_bytes(cur["subBytes"])
            out_str = " ".join(f"{b:02x}" for b in out_bytes)
            _mon_log.info(f"{pfx} cyc={cyc:5d}  *** [SBOX_DONE] *** subBytes output: {out_str}")
...
async def reset_dut(dut):
    ...
    dut.sbox_enb_n.value = SBOX_ENB_N_STATE_MATRIX
    dut.proceed.value = 1  # TB is the only consumer here, so always acknowledge sbox_done_pulse immediately
...
async def test_sbox_single_vector(dut):
    ...
    MONITORS:
    - ALL signal transitions (sbox_input, subBytes, sbox_done, FSM, etc.)
    ...
    # Wait for sbox_done
    dut._log.info(f"\n[STAGE 5] Waiting for sbox_done...")
    cycles = await wait_signal(dut.sbox_done_pulse, value=1, timeout=100, clk=dut.clk)
    dut._log.info(f" sbox_done asserted after {cycles} cycles")
...
async def test_sbox_pipelined(dut):
    """
    TC2: Pipelined test vectors using sbox_done signal.
    
    After initial setup and first sbox_done, drive next plaintext
    on the same cycle as sbox_done, then read output.
    """
    dut._log.info("=" * 90)
    ...
    for vec_idx in range(len(test_vectors)):
        # Wait for sbox_done
        dut._log.info(f"  Vector {vec_idx}: Waiting for sbox_done...")
        await wait_signal(dut.sbox_done_pulse, value=1, timeout=100, clk=dut.clk)
