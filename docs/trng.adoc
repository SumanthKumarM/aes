...
*Runtime Error Handling:* If either test fails, the TRNG immediately transitions to an **unrecoverable DEAD state**, halting all key generation. The device requires an external reset (`ext_rst_n`) to resume operation. No error recovery is attempted — any detected anomaly is treated as a critical malfunction.
...
The Control Unit manages the TRNG state machine with a *fail-fast error handling model*:

* *IDLE:* Warm-up state; noise source and health tests are disabled; keccak held in reset.
* *BIST:* Built-in Self-Test; health tests monitor the raw bitstream; keccak processes entropy.
  - If `health_error` asserted → immediately transition to DEAD (no recovery).
* *WAIT_FOR_XFER:* Awaiting handshake from Sbox; key output is available.
  - If `health_error` asserted → immediately transition to DEAD.
* *DEAD:* Unrecoverable failure state; all outputs disabled; device halts key generation.
  - Remains in DEAD until external reset (`ext_rst_n` assertion).

*Design Philosophy:* Errors are fatal. This simplifies the implementation and ensures that any detected anomaly immediately prevents further random generation, protecting against side-channel leakage from a compromised entropy source.
